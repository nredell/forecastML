...
#'
#' @param data A data.frame with the target to be forecasted plus features/predictors.
#' @param type The type of dataset to return.
#' @param outcome_cols The column index of the target to be forecasted; only univariate forecasting is supported at present.
...
#' @param lookback The lag--in dataset rows--for each predictor. Lags that don't support direct forecasting for a given
#' horizon are dropped. Either 'lookback' or 'lookback_control' need to be specified.
#' @param lookback_control A list of vectors, specifying potentially unique lags for each predictor. The length
...
create_lagged_df <- function(data, type = c("train", "forecast"), outcome_cols = 1,
  ...
  if (!is.null(lookback)) {
    lookback_max <- max(lookback)
  } else {
    lookback_max <- max(unlist(lapply(lookback_control, max)))
  }
  ...
  if (type == "train") {
    ...
    data_out <- lapply(horizon, function(i) {
      ...
      if (!is.null(lookback)) {
        lookback_over_horizon <- lookback[lookback >= i]
      }
      ...
      data_x <- lapply(1:ncol(data), function(j) {
        ...
        if (!is.null(lookback_control)) {
          lookback_over_horizon <- lookback_control[[j]][lookback_control[[j]] >= i]
        }
      ...
      data_out <- data_out[-(1:lookback_max), ]

      attr(data_out, "horizon") <- i
      if (!is.null(lookback)) {
        attr(data_out, "lookback") <- lookback_over_horizon
      } else {
        attr(data_out, "lookback") <- lapply(seq_along(lookback_control), function(z) {lookback_control[[z]][lookback_control[[z]] >= i]})
      }
      data_out
    })
  }
  ...
  if (type == "forecast") {
    ...
    data_out <- lapply(horizon, function(i) {
      ...
      attr(data_x, "horizon") <- i
      if (!is.null(lookback)) {
        attr(data_x, "lookback") <- lookback_over_horizon
      } else {
        attr(data_x, "lookback") <- lapply(seq_along(lookback_control), function(z) {lookback_control[[z]][lookback_control[[z]] >= i]})
      }
      data_x
    })
  }
...
plot.lagged_df <- function(object) {
  ...
  lookback_max <- max(unlist(lookback))
  ...
  if (lookback_per_predictor) {
    ...
    data_horizon <- lapply(seq_along(lookback), function(i) {
      ...
      data_predictor <- lapply(seq_along(lookback[[i]]), function(j) {
        ...
        if (length(lookback_predictor) > 0) {
      ...
      #data_horizon$direct_forecast_horizon <- horizon[i]
      data_horizon
    })
