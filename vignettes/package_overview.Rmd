...
```{r}
plot(data_error, data_results, type = "global", facet = ~ horizon)
```
...
plot(data_error, data_forecast, facet = ~ horizon, type = "window")

plot(data_error, data_forecast, facet = ~ horizon, type = "horizon")

plot(data_error, data_forecast, facet = ~ horizon, type = "global")
```
...
plot(data_error, data_forecast, facet = ~ horizon)
```
...
DT::datatable(return_error(data_combined, data_actual, actual_indices)$error_by_horizon, options = list(scrollX = TRUE))
```
