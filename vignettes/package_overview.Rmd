...
We'll create a list of datasets, one for each forecast horizon.
...
created for each forecast horizon/model. The `lookback` argument in `forecastML::created_lagged_df()` was set to 
...
hyperparameters and model having different coefficients--if available--from the inner cross-validation loop. 
...
use entirely different ML algorithms to produce the short- and long-term forecasts.
...
* A user-defined wrapper function for model training that takes the following arguments: 
    + **1:** A horizon-specific data.frame made with `forecastMLcreate_lagged_df(..., type = "train")` 
    (e.g., my_lagged_df$horizon_h),
    + **2:** optionally, any number of additional named arguments which can be passed as '...' in
    `forecastML::train_model()` or set with default arguments in the model function.
...
* **model:** User-supplied model name in `forecastML::train_model()`.
...
* **valid_indices:** Validation dataset row names from `attributes(forecastML::create_lagged_df())$row_indices`.
* **date_indices:** If given, validation dataset date indices from `attributes(forecastML::create_lagged_df())$date_indices`.
* **"groups":** If given, the user-supplied groups in `forecastML::create_lagged_df()`.
...
plot(data_results, type = "forecast_stability", valid_indices = attributes(data_list)$date_indices[1:3])
```
...
1. Error by validation window; this is the horizon-specifc error
2. Error by model forecast horizon, collapsed across validation windows (only useful when multiple forecast horizons are trained together)
...
### Forecast with nested cv models

#### forecastML::create_lagged_df

To forecast with the direct forecasting method, we need to create another dataset of forward-looking features. 
We can do this by running `forecastML::create_lagged_df()` and setting `type = "forecast"`.
