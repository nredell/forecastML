...
Let's view the modeling dataset for a forecast horizon of 6. Notice that "_lag_lookback" has been appended to all model features. Grouped and 
...
Below is a plot of forecast error metrics for collapsed across validation windows.
...
The `index` column gives the row index for the most recent period of actuals. In the example below, the `index` of 180 
represents the last row in our model-building dataset `data_train`, and the `index` of 181 is the first forecast period in our hold-out test dataset.
