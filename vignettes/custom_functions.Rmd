...
* View the `return()` values from the user-defined `model_function()`. The returned vales are stored in 
...
is the returned value of the user-defined modeling function; here, a list with 2 elements. The second parameter is the horizon-specific dataset with model features from `create_lagged_df()` (`type = "train"` or `type = "forecast"`).
